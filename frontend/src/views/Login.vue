    ...
    <div class="auth-right">
      <div class="auth-form-box">
        <div class="form-header">
          ...
          <p class="form-tip">欢迎使用苹果叶片病害检测系统</p>
        </div>
...
const getLastUsername = () => {
  return localStorage.getItem('last-username') || ''
}
...
// 页面加载时自动填充用户名
onMounted(() => {
  loginForm.username = getLastUsername()
})
  ...
  await loginFormRef.value.validate((valid) => {
    if (valid) {
      ...
      api.login(loginForm).then(res => {
        localStorage.setItem('apple-disease-token', res.data.token)
        // 保存用户信息
        localStorage.setItem('user-info', JSON.stringify(res.data.user_info))
        // 保存登录用户名
        localStorage.setItem('last-username', loginForm.username)
